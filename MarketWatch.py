...
class MarketWatch(tk.Frame):
    def __init__(self, window, obj):
        ...
        menu.pack()
        mktmycursor.execute("select user_id from users where user_name = \"{}\"".format(obj.uname))
        stock_list = []
        uid = mktmycursor.fetchone()[0]
        print(uid)
        mktmycursor.execute("select sector_id,sector_name from sector")
        secid = []
        sec_name = []
        dict1 = {}
        c = 0
        for i in mktmycursor.fetchall():
            secid.append(i[0])
            sec_name.append(i[1])
        ...
        def putin(sel_sec_id):
            ...
            sql = "select stock_id,stock_name,pps,quantity from Stocks where sector_id = {} "
            mktmycursor.execute(sql.format(sel_sec_id))
            print("look",sel_sec_id)
            stock_id = []
            stock_name=[]
            pps=[]
            quantity=[]
            for row in mktmycursor.fetchall():
